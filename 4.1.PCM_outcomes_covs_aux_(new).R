...
permute <- function(df) { 
  ...
  new_n <- summary(as.factor(perm$new_groups))[4]
  return(new_n)
}
...
cov_out$int.age.10y      <-  as.numeric(levels(alspac.table$kv9991a))[alspac.table$kv9991a]/12  # age 10 (months)
## SHOULD THIS NOT BE TRANSFORMED IN YEARS????
cov_out$intern_score.10y <-  as.numeric(levels(alspac.table$kv8603))[alspac.table$kv8603]    # age 10 

#no number 2 in intern_score.10y

# cov_out$int.age.13y      <- as.numeric(levels(alspac.table$tb9991a))[alspac.table$tb9991a] # age 13 
# cov_out$intern_score.13y <- as.numeric(levels(alspac.table$tb8603))[alspac.table$tb8603]   # age 13    
# cov_out$int.age.15y      <- as.numeric(levels(alspac.table$fh0011a))[alspac.table$fh0011a] # age 15 
# cov_out$intern_score.15y <- as.numeric(alspac.table$fh6876)                                # age 15    
...
cov_out$fat_mass.10y <- as.numeric(levels(alspac.table$f9dx126))[alspac.table$f9dx126] # trunk FM at age 10y
## IS TRUNK AND ANDROID FAT MASS THE SAME THING, IS ANDR @10 NOT AVAILABLE
...
cov_out$intern_score_z.10y <- as.numeric(scale(cov_out$intern_score.10y))
...
cov_out$fat_mass_z.10y <- as.numeric(scale(cov_out$fat_mass.10y))
...
cov_out$int = ifelse(cov_out$intern_score_z.10y > quantile(cov_out$intern_score_z.10y, probs = 0.8, na.rm = T), 1, 0) 
cov_out$fat = ifelse(cov_out$fat_mass_z.10y     > quantile(cov_out$fat_mass_z.10y,     probs = 0.8, na.rm = T), 1, 0) 
...
attach(cov_out); plot(intern_score_z.10y, fat_mass_z.10y, col=c("cornflowerblue","black", "red", "darkgrey", "darkgoldenrod2","chartreuse4")[risk_groups]); detach(cov_out)
...
for (i in 1:itarations) {
  if (permute(cov_out) > summary(cov_out$risk_groups)[4]) {
    count <- count + 1
  }
}
...
## BMI ONLY DURING PREGNANCY AVAILABLE
...
# ------------------------------------------------------------------------------
### SEX of the child
cov_out$sex <- alspac.table$kz021 # CODING?????? 1 = boy; 2 = girl.
...
cov_out$age_child.10y <- (cov_out$int.age.10y + cov_out$fm.age.10y) / 2
...
# 2 categories (sustained vs (stopped or never))

cov_out$m_smoking <- ifelse(alspac.table$e170 =='N', 0, 
                            ifelse(alspac.table$e170 =='Y', 1, NA))

#-------------------------------------------------------------------------------
### MATERNAL ALCOHOL CONSUMPTION during pregnancy
# We chose the earliest timepoint available for maternal drinking during pregnancy, 
# as effects of alcohol are most potent in first trimester, and more is data available
## WHY ARE THERE TWO TIMEPOINTS THEN? WHAT ARE THOSE? 

cov_out$b721r <- ifelse(alspac.table$b721 == 'never', 0, 
                        ifelse(alspac.table$b721 == '<1 glass PWK', 1,
                               ifelse(alspac.table$b721 == '1+ glasses PWK', 2, NA)))

cov_out$e220r <- ifelse(alspac.table$e220 == 'Not at all', 0, 
                        ifelse(alspac.table$e220 == '<1PWK', 1,
                               ifelse(alspac.table$e220 == 'At least 1PWK', 2, NA)))
...
# Maternal weight (Kg)
cov_out$weight_pre <- as.numeric(levels(alspac.table$dw002))[alspac.table$dw002] # kg 12w gest
...
# BMI
cov_out$m_bmi_pregnancy <- cov_out$weight_pre / ((cov_out$height_7y)^2) # calculating pregnancy BMI 
...
cov_out$sibling <- ifelse(alspac.table$mult == 1, 1, ifelse(alspac.table$mult == 0, 0, NA))

## NO NEED FOR NUMERIC, NO??
